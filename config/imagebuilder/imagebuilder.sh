...
custom_packages() {
    ...
    if [[ "${IMAGEBUILDER_SKIP_CUSTOM_PACKAGES:-1}" == "1" ]]; then
        echo -e "${INFO} Custom package download disabled for original/minimal image."
        return 0
    fi
...
rebuild_firmware() {
    ...
    # Keep the ImageBuilder profile original. PACKAGES only adds the explicit
    # package list from config/imagebuilder/config on top of profile defaults.
